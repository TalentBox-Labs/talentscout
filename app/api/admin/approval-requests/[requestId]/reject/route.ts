...
export async function POST(request: Request, context: { params: Promise<{ requestId: string }> }) {
  ...
  if (!reviewNotes) {
    return NextResponse.json({ message: "Review notes are required when rejecting a request." }, { status: 400 });
  }

  const { response: backendResponse, auth } = await adminBackendFetch(
    `/api/v1/admin/approval-requests/${requestId}/reject?review_notes=${encodeURIComponent(reviewNotes)}`,
    {
      method: "POST",
    },
  );
  ...
  if (!backendResponse.ok) {
    return applyBackendAuth(
      NextResponse.json(
        { message: extractErrorMessage(data, "Failed to reject request.") },
